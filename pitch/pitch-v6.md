# Cadence — pitch v6.1 (launch-dossier integration + real-bench panel-4 fixes)

> v6.1 deltas (panel 4, real 5-judge bench, 46.4/60): Dana's arithmetic fixed ($22.50/wk → $45/fortnight);
> the settle-beat CANDOUR LINE moved into the spoken script (Fiserv-VP persona: "the most hireable 15
> seconds of the night"); monetisation slide made spike-honest (applicationFee accepted 201 on a direct
> merchant — routing to a third party is the partner conversation; rev-share is the clean fallback);
> rail-agnostic close beat added. Outstanding judge asks that are Shivam-side: external legal read on
> SMS-consent-as-variation; one real merchant LOI / anonymised dishonour export.
...
Dana's gym debits her forty-five dollars a fortnight. Last Tuesday the debit bounced — three days before payday — and her gym's legacy biller charged her **twenty-nine dollars ninety**. **[pause]** More than a week of her membership, for being three days early. For the incumbent billers, the failure *is* the revenue. The gym never noticed. Dana quit.
...
Watch. *(risk sweep)* The model prices Dana's Thursday debit at sixty-four percent — drag it; every date has a price, and this curve was learned. *(bank date)* Dishonoured. *(the ask)* The model picks her funded day and texts her — I'll type her reply: *"can't do Friday, Monday works."* **[silence]** Caught the negation. Monday. *(settle)* One honest sentence first: payer, source, debit and this model-timed recovery are **live API calls — real IDs on screen**; the settlement event is the one thing this sandbox won't fire on demand, so it's labelled simulated, and the spike scripts proving that are in the repo. *(beat)* **Settled. Forty-five dollars recovered** — and the transfer reconciles it on Pinch's rails: our fifteen percent is billed only on the save — and the applicationFee field is live-verified on this rail; whether it settles as a platform split or a monthly rev-share bill is precisely the partner conversation. **We only earn when the payment lands. The incumbents earn when it fails — which is exactly why they can't copy this.**
...
And the mechanism is rail-agnostic: gate, score, consent, receipt — **BECS via Pinch today; Clover recurring and US ACH are the same loop**, sitting exactly where card-decline optimisation stops. **Twenty-nine-ninety for failing — or forty-five dollars recovered, with a receipt.** The repo is public. Let's run the pilot.

## The 2:00 compression (~270 words)

Dana's gym debits her forty-five dollars a fortnight. It bounced three days before payday, and the legacy biller charged her **twenty-nine dollars ninety** — more than a week of her membership. **[pause]** For the incumbents, the failure *is* the revenue.
...
Watch: the model prices her debit at sixty-four percent risk. It fails. It picks her funded day and texts her — I'll type her reply live: *"can't do Friday, Monday works."* **[silence]** Caught the negation. Monday. Everything you've seen is a live API call except the settlement tick — the sandbox won't fire it on demand; it's labelled simulated, receipts in the repo. **Settled — forty-five dollars recovered**, our fifteen percent billed only on the save. **We earn only when the payment lands; the incumbents earn when it fails — that's why they can't copy this.**
...
- **"applicationFee only splits for managed sub-merchants — show me it landing on a merchant you did NOT onboard."** → *"You're right about the routing model, and we spiked exactly that question: on a standard direct merchant, the API accepts the field — 201, fee echoed, merchant totalFee reflects it (spike script in the repo). Whether it settles to the application owner outside the managed-merchant model is precisely what we'd resolve in the partner conversation — and if the answer is no, the clean fallback is a monthly rev-share bill computed from the same attributed-save ledger. The pricing model doesn't depend on the plumbing; the plumbing question is real and we brought it to you rather than hand-waving it."*
- **"Who answers the phone at 9pm on a disputed re-timed debit? You carry 15% and none of the operational risk."** → *"Today: the merchant's existing Pinch dispute path — Cadence never touches funds and every re-time carries its consent receipt, which is the artefact that resolves that call in minutes instead of weeks. At pilot scale I'm the escalation contact. And that risk asymmetry is exactly why the right structure long-term is WITH Pinch — inside your standing, your DDRSA, your support — not around you. That's ask number one."*
