...
| 18–30s | Click **Run to bank date** → red DISHONOURED. | "It fails — insufficient funds. Instead of a blind retry, Cadence predicts the payer's funded day and asks." |
| 30–42s | **Type as Dana:** `cant do friday, monday works`. Parser → Monday. | "Watch — I'll type her reply. The parser catches the negation, picks Monday, and stamps her exact words into the payment's metadata as a consent receipt." |
| 42–54s | Point at the API pane: real `POST /payments`, real `pmt_` id, `applicationFee: 675`. | "That just created a real recovery payment on Pinch — with our 15% riding as a native applicationFee. We only earn when the payment actually lands." |
| 54–60s | Cut to `/merchant.html` (the risk-ranked run). | "Predict, re-time, recover — the failed-payment brain Pinch's own docs say merchants need. That's Cadence." |
